...
public class BrowserElements extends BaseTestSetup {
     ...
     */
    public void waitForWebElementIsDisplayed(final WebElement element, final boolean waitForNotDisplayed) {
        int retryCount = 5;
        while (retryCount > 0) {
            try {
                (new WebDriverWait(webDriver, Long.parseLong(AutProperties.getRequestTimeOut())))
                ...
                if (retryCount != 0) {
                    try {
                        ...
                        Assert.fail("Exception while waiting for elements to be displayed", ite);
                    }
                }
     ...
     * @param element
     */
    public void waitForWebElementIsDisplayed(final WebElement element) {
    ...
    public void clickElementAfterFocus(WebElement element) {
        clickElementAfterFocus(element, true);
    }
     ...
     *
     * @param element         WebElement to be clicked.
     * @param scrollToElement - If true, scroll to bottom of the page after shifting focus
     */
    public void clickElementAfterFocus(WebElement element, boolean scrollToElement) {
        ...
        while (retryCount > 0) {
            try {
                ...
                Logger.error("Stale elements exception while waiting clicking on elements: " + element
                        .toString(), e);
                retryCount--;
                if (retryCount != 0) {
                    try {
                ...
                Assert.fail(
                        "StaleElementException while clicking selenium elements" + element.toString(), e);
            } catch (Exception ex) {
                ex.printStackTrace();
    ...
    public void clickButtonByText(String buttonText) throws Exception {
        ...
        WebElement buttonXPath = webDriver.findElement(By.xpath("//button[contains(.,'" + buttonText + "')]"));
     ...
     */
    public void waitForWebElementIsEnabled(WebElement webElement, final boolean waitForNotEnabled) {
        int retryCount = 5;
        while (retryCount > 0) {
            try {
                (new WebDriverWait(webDriver, Long.parseLong(AutProperties.getRequestTimeOut())))
                ...
                Assert.fail("Time out while waiting for an elements " + webElement + " to be enabled!" + timeOut);
            } catch (Exception exception) {
                exception.printStackTrace();
                Assert.fail("Failed to wait for elements to be enabled. " + exception, exception);
            }
        }
    }
    ...
    public boolean isElementDisplayed(By byElement) {
        boolean isDisplayed;
        try {
            isDisplayed = webDriver.findElement(byElement).isDisplayed();
        } catch (Throwable e) {
            isDisplayed = false;
        }
        return isDisplayed;
    ...
    private void waitForElementTobeClickable(By byElement) {
        WebDriverWait wait = new WebDriverWait(webDriver, Long.parseLong(AutProperties.getRequestTimeOut()));
    ...
    public String getAttribute(By byElement, String attribute) throws Exception {
        ...
        while (retryCount > 0) {
            try {
                ...
                attrVal = webDriver.findElement(byElement).getAttribute(attribute);
                break;
            } catch (StaleElementReferenceException sere) {
